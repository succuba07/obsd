...
class EfficientNetFeatureExtractor(nn.Module):
    def __init__(self):
        ...
        original_net = models.efficientnet_b0(weights=models.EfficientNet_B0_Weights.IMAGENET1K_V1)
        
        self.features = nn.Sequential(
            original_net.features,
            original_net.avgpool
        )
    ...
    try:
        ...
        for fname in os.listdir(group_dir):
            ...
            if match:
                ...
                image_ids.append(f"{group_name}_0{stage}")
...
def main():
    ...
    # 保存特征文件
    with open("image_features_efficient.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["image_id"] + [f"feature_{i}" for i in range(1280)])
        for img_id, feat in all_features:
            writer.writerow([img_id] + feat.tolist())
    
    # 保存相似度记录
    with open("similarity_scores_efficient.csv", "w", newline="") as f:
        writer = csv.writer(f)
    ...
    plt.xlabel("阶段过渡")
    plt.ylabel("相似度")
    plt.title("甲骨文演化阶段相似度趋势（EfficientNet）")
    ...
    plt.tight_layout()
    plt.savefig('efficientnet_similarity.png', dpi=300, bbox_inches='tight')
    plt.show()
